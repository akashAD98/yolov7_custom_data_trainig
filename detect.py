...
def frames_to_timestamp(frame_num, fps):

#"""Converts the current frame number into a timestamp based on the video's
#fps. Anything over 24 hours is displayed accurately.
#Example for 100 hour timestamp: 100:00:00"""
            ...
            if len(det) and ((frame - (last_frame_detected + (SECONDS_DELAY_AFTER_DETECTION * fps))) >= 0):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    #    line = (cls, *xywh, conf) if opt.save_conf else (cls, *xywh)  # label format
                        ...
                        #sheet.append((cls[xywh], time_ref ))
                        #time_ref =time.time()
                        #print("time:",time_ref)
                        #print("###")
                   #     with open(txt_path + '.txt', 'a') as f:
                   #         f.write(('%g ' * len(line)).rstrip() % line + '\n')
                    print('timestamp',frames_to_timestamp(frame, fps))
                    print('frame',frame)
                    print('class',cls)
